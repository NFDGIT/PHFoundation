...
//  PHCommonDemo
//
//  Created by penghui on 2020/8/14.
...
public extension String {
    /// 正在输入的正则判断
    /// - Returns: 是否匹配
    func editingEmailMatch() -> Bool {
        return true
    }
    /// 输入完成的正则判断
    /// - Returns: 是否匹配
    func editedEmailMatch() -> Bool {
        let rules = NSPredicate(format: "SELF MATCHES %@", "^[A-Za-z0-9\\u4e00-\\u9fa5]+@[a-zA-Z0-9_-]+(\\.[a-zA-Z0-9_-]+)+$")
    ...
    /// 正在输入的正则判断
    /// - Returns: 是否匹配
    func editingPwdMatch() -> Bool {
        return true
    }
    /// 输入完成的正则判断
    /// - Returns: 是否匹配
    func editedPwdMatch() -> Bool {
        return true
    }
}
